...
browseraction.INSERT_EVENT_API_URL_ = 'https://www.googleapis.com/calendar/v3/calendars/{calendarId}/events';
/**
...
browseraction.installButtonClickHandlers_ = function() {
  ...
  $('#chrome-ext-parse').on('click', function() {
    console.log(1);
    chrome.tabs.query({ active: true, currentWindow: true }, function(tabs){
      chrome.tabs.sendMessage(tabs[0].id, {method: "events.parse"}, function(response) {});
    });
    // chrome.runtime.sendMessage({ method: 'events.parse' });
  });
...
browseraction.addNewEventIntoCalendar_ = function() {
  const end = $('#add-event-end').val().toString();
  const start = $('#add-event-start').val().toString();
  const summary = $('#add-event-summary').val().toString();
  browseraction.createInsertEvent_({
    end: {
      dateTime: end,
    },
    start: {
...
chrome.runtime.onMessage.addListener(function(request, sender, sendResponse) {
  if (request.method === 'add-event') {
    browseraction.createInsertEvent_(request.data);
  }
  sendResponse();
});
...
browseraction.createInsertEvent_ = function(
  data,
  calendarId = constants.DEFAULT_CALEDAR_ID
) {
  const insertUrl = browseraction.INSERT_EVENT_API_URL_.replace('{calendarId}', encodeURIComponent(calendarId));
